 ...
 var table = $('#example2').DataTable({
    ...
    "rowCallback": function (row, data, index) {
        ...
        if (data.status == 'Active'){
            $('td:eq(4)', row).html(
              '<a href="'+ 'assigncampaignchangestatus'+'/'+ data.id+'" class="btn btn-success">Enable</a>'
              );
        }

        else{
          $('td:eq(4)', row).html(
            '<a href="'+ 'assigncampaignchangestatus'+'/'+ data.id+'" class="btn btn-danger">Disable</a>'
            );
      }


      $('td:last-of-type', row).html(
        '<a class="btn btn-icon-only green" title="Edit" href="' + 'assigncampaign' + '/' + data.id  +'/'+'edit'+'"><i class="fa fa-edit"></i></a>' +
        ' <a type="button" id="assigncampaign' + data.id + '" title="Delete" data-id=' + data.id + ' class="btn btn-icon-only red delete_server"><i class="fa fa-trash"></i></a>'
        );
  },
